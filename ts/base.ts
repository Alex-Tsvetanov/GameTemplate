...
var gridSize: number = undefined; //50
...
function areColliding (Ax: number, Ay: number, Awidth: number, Aheight: number, Bx: number, By: number, Bwidth: number, Bheight: number): boolean 
{
    if (Bx <= Ax + Awidth) 
    {
        if (Ax <= Bx + Bwidth) 
